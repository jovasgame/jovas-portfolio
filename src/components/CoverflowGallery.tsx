...
import { getDirectThumbnailUrl, isGoogleDriveUrl } from "../utils/mediaUtils";
...
export const CoverflowGallery: React.FC = () => {
  ...
  const slides: Slide[] =
    projects && projects.length > 0
      ? projects.map((p) => ({
          image: {
            src: isGoogleDriveUrl(p.videoUrl) ? getDirectThumbnailUrl(p.videoUrl) : getDirectThumbnailUrl(p.imageUrl || p.videoUrl),
            alt: p.title,
          },
          title: `${p.title}\n${p.category} (${p.year})`,
          project: p,
  ...
  return (
    <section id="galeria-3d" className="py-20 relative bg-[#0a090c]/35 backdrop-blur-md border-t border-b border-white/5 overflow-hidden w-full max-w-full">
      {/* Background Orbs */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] max-w-full h-[300px] bg-gradient-to-r from-[#ff5540]/15 to-[#feba39]/15 rounded-full blur-[140px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Control Bar */}
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 max-w-xl mx-auto bg-[#1e1c21]/50 backdrop-blur-md border border-white/10 p-3 rounded-2xl shadow-xl">
